 ...
 */
function ttobias_form_system_theme_settings_alter(&$form, FormStateInterface $form_state, $form_id = NULL) {
  ...
  if (!Ttobias::isLocal()) {
    $versions = Ttobias::getCdnVersions('bulma');
    ...
    $version = Ttobias::getBulmaLocalVersion();
    $form['cdn']['bulma']['version'] = [
      '#type' => 'item',
      '#title' => t('Version'),
  ...
  if ($themes = Bulmaswatch::getThemes()) {
    ...
    if (!Bulmaswatch::isLocal()) {
      $versions = Ttobias::getCdnVersions('bulmaswatch');
 ...
 */
/*
function ttobias_form_system_theme_settings_validate($form, FormStateInterface $form_state) {
