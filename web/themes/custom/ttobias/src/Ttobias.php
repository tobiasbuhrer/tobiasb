...
/**
 * The primary class for the Drupal ttobias base theme.
 *
...
class Ttobias {
    ...
    if (!empty($bulma_button)) {
      foreach ($bulma_button as $key => $class) {
        ...
        if ($key == 'colorize') {
          ...
          // Get class.
          $color = Ttobias::cssClassFromString($name, 'is-primary');
  ...
  public static function iconMatch($name, $type) {
    ...
    $icons = [
      ...
      t('titre')->render() => 'pencil',
      t('Subject')->render() => 'pencil',
      t('Sujet')->render() => 'pencil',
      t('Changer votre message')->render() => 'pencil',
      ...
      t('Aperçu avant envoi')->render() => 'eye',
