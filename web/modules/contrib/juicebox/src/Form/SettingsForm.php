...
class SettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['multilingual']['base_languagelist_suggestion'] = array(
      '#type' => 'item',
      '#title' => $this->t('Suggested base string for currently detected Juicebox version (@version)', array('@version' => $version)),
    ...
    if ($multisize_disallowed) {
      $multisize_description .= '<p><strong>' . $this->t('Your currently detected Juicebox version (@version) is not compatible with multi-size features, so the options below have been disabled.', array('@version' => $version)) . '</strong></p>';
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getvalue('translate_interface') && !empty($library['installed']) && $form_state->getvalue('base_languagelist') != $library['base_languagelist']) {
      drupal_set_message($this->t('Interface translations are enabled but the base translation string does not match the suggested value for your version of the Juicebox javascript library. If some parts of the Juicebox interface do not appear translated correctly please verify that your base translation string is correct.'), 'warning');
    }
    ...
    drupal_set_message($this->t('The Juicebox configuration options have been saved'));
  }
