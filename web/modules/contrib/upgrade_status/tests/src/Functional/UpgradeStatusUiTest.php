...
class UpgradeStatusUiTest extends UpgradeStatusTestBase {
  ...
  public function testUiAfterScan() {
    ...
    $this->assertSame($this->getDrupalCoreMajorVersion() < 10 ? 'No problems found' : '2 problems', strip_tags($page->find('css', 'tr.project-upgrade_status_test_submodules td.scan-result')->getHtml()));
    ...
    $this->assertSame($this->getDrupalCoreMajorVersion() < 10 ? 'No problems found' : '1 problem', strip_tags($page->find('css', 'tr.project-upgrade_status_test_contrib_10_compatible td.scan-result')->getHtml()));
    ...
    $this->assertSession()->linkByHrefExists('https://drupal.org/project/issues/upgrade_status_test_contributed_10_compatible?text=Drupal+' . $next_major . '&status=All');
    ...
    $edit = [
      ...
      ($this->getDrupalCoreMajorVersion() < 10 ? 'relax' : 'manual') . '[data][list][upgrade_status_test_10_compatible]' => TRUE,
    ...
    foreach ($expected as $button => $assert) {
      ...
      $this->assertSession()->pageTextContains('Upgrade status test 10 compatible');
      $this->assertSession()->pageTextContains('Upgrade status test error');
