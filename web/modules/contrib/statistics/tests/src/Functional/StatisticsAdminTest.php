...
class StatisticsAdminTest extends BrowserTestBase {
  ...
  public function testDeleteNode() {
    ...
    $result = $connection->select('node_counter', 'n')
      ...
      ->execute()
      ->fetchAssoc();
    $this->assertEquals($result['nid'], $this->testNode->id(),
  ...
  public function testExpiredLogs() {
    $this->config('statistics.settings')
      ...
      ->save();
    $this->state->set('statistics.day_timestamp', 8640000);
    ...
    // \Drupal::time()->getRequestTime() in the delete query, so wait two secs
    // here to make sure the access log will be flushed for the node just hit.
    sleep(2);
    $this->cronRun();
    ...
    $this->assertSession()
      ->pageTextNotContains('node/' . $this->testNode->id());
